...
public:
    ...
    alloc_unique_trait_ptr(alloc_unique_trait_ptr const&)            = delete;
    alloc_unique_trait_ptr& operator=(alloc_unique_trait_ptr const&) = delete;
...
private:
    ...
    friend auto allocate_unique_trait(Alloc const&, Args&&...);
...
[[nodiscard]] auto trait_cast(alloc_unique_trait_ptr<Trait>&& ptr) -> alloc_unique_trait_ptr<Supertrait> {
    if (not ptr)
        return {};
    auto new_ptr =
        alloc_unique_trait_ptr<Supertrait>(trait_cast<Supertrait>(ptr.dyn_trait_ref_), ptr.get_ctrl_ptr());
    ptr.release();
    return new_ptr;
}
template<any_trait T, implements_trait<T> Impl, supertrait_of<T> U>
[[nodiscard]] auto trait_cast(alloc_unique_trait_ptr<U>&& ptr) -> alloc_unique_trait_ptr<T> {
    if (not ptr or not is_holding_type<Impl>(ptr))
        return {};
    auto new_ptr = alloc_unique_trait_ptr<T>(trait_cast<T, Impl>(ptr.dyn_trait_ref_), ptr.ctrl_ptr_);
    ptr.release();
    return new_ptr;
}
...
[[nodiscard]] auto is_holding_type(alloc_unique_trait_ptr<Trait> const& ptr) -> bool {
    return ptr and is_holding_type<Impl>(*ptr);
}

template<any_trait Trait, implements_trait<Trait> Impl, typename Alloc, typename... Args>
[[nodiscard]] auto allocate_unique_trait(Alloc const& allocator, Args&&... args) {
...
public:
    ...
    unique_trait_ptr(unique_trait_ptr const&)            = delete;
    unique_trait_ptr& operator=(unique_trait_ptr const&) = delete;
...
template<typename Impl, any_trait Trait>
    requires implements_trait<Impl, Trait>
[[nodiscard]] auto is_holding_type(unique_trait_ptr<Trait> const& ptr) -> bool {
    return ptr and is_holding_type<Impl>(*ptr);
}
