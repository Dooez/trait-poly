...
inline constexpr auto by_type_value         = 2;
...
inline constexpr auto emplace_index_value   = 13;
...
void check_construction() {
    ...
    auto by_type = lifecycle_variant(std::in_place_type<other_impl>, by_type_value);
    test::expect_eq(case_name, "in-place type index", static_cast<int>(index(by_type)), 1);
    test::expect_eq(case_name, "in-place type value", by_type.value(), by_type_value);
...
void check_emplace() {
    ...
    auto& second = emplace<1>(var, emplace_index_value);
    test::expect_eq(case_name, "emplace index ref", second.value(), emplace_index_value);
    test::expect_eq(case_name, "emplace index value", var.value(), emplace_index_value);
}
