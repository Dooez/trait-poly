...
static_assert(
    std::same_as<decltype(get_if<first_index>(std::declval<variant_t const*>())), first_impl const*>);
static_assert(
    std::same_as<decltype(emplace<first_index>(std::declval<variant_t&>(), first_value)), first_impl&>);
static_assert(
    std::same_as<decltype(emplace<second_impl>(std::declval<variant_t&>(), by_index_value)), second_impl&>);
...
inline void run() {
    ...
    test::expect_eq(case_name, "get index", get<first_index>(var).stored, first_value);
    test::expect_eq(case_name, "get type", get<first_impl>(var).value(), first_value);
    test::expect_eq(case_name, "get_if index", get_if<first_index>(&var)->stored, first_value);
    test::expect_eq(case_name, "get_if type", get_if<first_impl>(&var)->stored, first_value);
    ...
    test::expect_eq(
        case_name, "emplace index returns active ref", replaced_by_index.stored, emplace_index_value);
    ...
    test::expect_eq(
        case_name, "emplace type returns active ref", replaced_by_type.stored, emplace_type_value);
    ...
    test::expect_eq(
        case_name, "same alternative assignment index", static_cast<int>(index(var)), first_index);
