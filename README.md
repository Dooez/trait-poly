...
Since the dot notation is reserved for trait method access, external to the trait interaction with `dyn_trait_ref` is provided via free functions:
...
- `trp::trait_cast<ExplicitSupertrait>(ref) -> dyn_trait_ref<ExplicitSupertrait>` 
    Any trait vtable includes all direct supertrait vtables. This conversion is always valid.
- `trp::trait_cast<AnotherTrait, Impl>(ref)` returns a trait reference assuming the object is `Impl`.
...
`/* method-holder */` is a standard-layout class with a first and only data member of type `method_invoker<...>`.  
...
1. The `this` pointer of `method_invoker<...>::operator()(...)` is `reinterpret_cast` to `/* method-holder */` pointer (well-defined because `/* method-holder */` is a standard-layout struct with a single member).
2. `/* method-holder */` pointer is `static_cast` to `/* dyn-ref-wrapper */` pointer (well-defined due to inheritance).

Through the `/* dyn-ref-wrapper */` pointer, the vtable and type-erased object pointers are acquired.
...
## Build examples
- Configure with the p2996 preset: `TRP_P2996_INSTALL_PATH=/path/to/clang-p2996/install cmake --preset clang-p2996`
